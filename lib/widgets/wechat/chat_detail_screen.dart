...
class ChatDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final details = interupts.wechat_notifications.firstWhere(
          (e) => e["who"] == person,
          orElse: () => {"details": []},
        )["details"] ?? [];
    return Scaffold(
      appBar: AppBar(title: Text("$person on $app"),
              actions: [
                  IconButton(
                    icon: Icon(Icons.open_in_new),
                    tooltip: "Open in WeChat",
                    onPressed: () async {
                      // 1. Delete messages from storage
                      await WechatStorageService.deleteNotifications(person);
                      // 2. Open WeChat via the link
                      await openWeChatApp();
                    }
